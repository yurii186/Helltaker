...
class GameTimer {
    ...
    getScore() {
        const secondsPassed = (this.endTime - this.startTime) / 1000; // Получаем количество прошедших секунд
        console.log("SCORE: ", secondsPassed)
        let score = Math.floor(100 - secondsPassed); // Рассчитываем начальное количество очков
        score = Math.max(0, score); // Ограничиваем количество очков до 0
        return Math.min(100, score); // Ограничиваем количество очков до 100
    }
}
...
function GamePlay({chapterLevel}) {
    useEffect(() => {
        console.log("LEVEL GAMEPLAY = ", chapterLevel);
    }, [chapterLevel]);
    const maxLevel = 7;
    ...
    const sendKeyPressToServer = async (key, currentLevelBoard, level) => {
        ...
        try {
            const postResponse = await fetch('http://localhost:8080/game/move', {
                ...
                body: JSON.stringify({gameBoard: currentLevelBoard, level: level, input: key})
            });
            ...
            //     await handleMoveDown();// сделай на джаве что нужно менять с какой анимацией потом через запрос получишь значение и поймёшь какую анимацию
            //использовать также сделай много функций на каждую анимацию и потом создай одну функцию которая будет определять какую анимацию использовать
            //await handleMoveUp();
            // await handleMoveLeft();
            // await handleMoveRight();


            //сделай потом если бьёшь склета в камень то скелет просто применяет анимацию и потом исчезает
            // и для камня тоже чтобы была лишь анимация удара если за ним камень
            ...
            if(steps === -4) {
                ...
                if (response.ok) {
                    if (Array.isArray(data)) {
                        const newScore = gameTimer.getScore(); // получаем счет из экземпляра gameTimer
                        setGameState({
                ...
                {Array.isArray(gameState.currentLevelBoard) && gameState.currentLevelBoard.map((row, rowIndex) => (
                    <div key={rowIndex} className="game-row">
                        {Array.isArray(row) && row.map((cell, cellIndex) => (
                            ...
                            >
                                {cell === '■' ?
                                    <div className="rock-sprite" id={`rock-${rowIndex}-${cellIndex}`}>
                                    </div> :
                                    cell === '♀' ?
                                        <div
                                        ...
                                        cell === '☠' ?
                                            <div
                                                id={`skeleton-${rowIndex}-${cellIndex}`} // Добавление идентификатора для каждого скелета
                                                className={`skull-sprite ${
                                                    actionScelet === 'idle' ? 'idle-shake-skull' : ''
                                                }`}
                                            ></div> :
                                            cell === '❤' ?
                                                <div className={`heart-sprite idle-level-${gameState.level}`}></div> :
                                                cell === '☰' ?
                                                    <div className={`spike-sprite`}></div> :
                                                    cell === '◙' ?
                                                        <div className={`spike-and-rock-sprite`} id={`spike-and-rock-${rowIndex}-${cellIndex}`}></div>
                                                        : null
                                }
                            </div>
                        ))}
